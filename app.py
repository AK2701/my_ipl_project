...
category = st.sidebar.radio(
    "Choose your option?",
    ('Baller Analysis', 'Batsman Analysis','Match Anaysis' ,'Win Prediction'))

if category == ' Baller Analysis':
    st.sidebar.write('You selected Baller Analysis.')
...
bowler_names = bowler_graphs['bowler'].unique().tolist()
...
if load or st.session_state.load_state:
    ...
    if category == 'Baller Analysis':
        ...
        if sort_by == 'Economy':
            ...
            bowler4 = bowler.sort_values(by='num_times_critical_wicket_taken', ascending=False).reset_index(drop=True)
        ...
        with col1:
            ...
            plot = sns.catplot(x='bowler', y='wic_taken', hue='season', kind='bar', data=filtered_data, palette=colors)
       ...
       # plot.set_ylim(0, 80)
        ...
        ax=sns.barplot(y=most_successfull.Team,x=most_successfull.matches_win)
        ax.set_xlim(0,110)
        ...
        ax=sns.countplot(y='venue',data=matches)
        st.pyplot()
        ...
        pipe = pickle.load(open('win.pkl', 'rb'))
        col1, col2 = st.columns(2)
